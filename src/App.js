...
const getSearchedMovies=async (word)=>{
 ...
 const getPage=async (page)=>{
  const res=await axios.get(`https://api.themoviedb.org/3/person/popular?language=ar&page=1&api_key=675e2d4e494c3d2d212e5237fe047d61&page=${page}`)
  setmovies(res.data.results)
  setisSearch(false)
  setTotalPages(res.data.total_pages)
...
<MoviesList movies={movies} isSearchParam={isSearch} pageCount={totalPages}
getPage={getPage}></MoviesList>
  </div>)
